...
def make_long_short_buckets(
    ...
    # Sélection initiale par quantile
    long_candidates  = signal[signal >= threshold_long].sort_values(ascending=False)
    short_candidates = signal[signal <= threshold_short].sort_values(ascending=True)

    # ── Market-neutral : forcer n_long == n_short ──
    n = min(len(long_candidates), len(short_candidates))
    if n == 0:
        return [], []

    long_list  = long_candidates.iloc[:n].index.tolist()
    short_list = short_candidates.iloc[:n].index.tolist()

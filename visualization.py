...
def _build_portfolio_dataframe(weights: pd.Series, signal_name: str) -> pd.DataFrame:
    """
    Construit un DataFrame du portefeuille final pour Bloomberg.
    
    Parameters
    ----------
    weights : pd.Series avec index = ticker, values = weight
    signal_name : nom du signal (pour info)
    
    Returns
    -------
    DataFrame avec colonnes: Ticker, Weight, Weight %
    """
    portfolio_items = []

...
def run_backtest(
    ...
    for date in rebal_dates:
        ...
        # ── 5b. Re-équilibrage market-neutral après stop-loss ──
        # Le stop-loss peut supprimer des positions d'un seul côté,
        # cassant l'équilibre n_long == n_short. On force la symétrie.
        active_long  = weights[weights > 0]
        active_short = weights[weights < 0]
        n_active = min(len(active_long), len(active_short))

        if n_active == 0:
            # Plus aucune position active → skip cette date
            continue

        if len(active_long) != len(active_short):
            # Garder les n_active positions les plus grosses de chaque côté
            keep_long  = active_long.abs().nlargest(n_active).index
            keep_short = active_short.abs().nlargest(n_active).index
            # Mettre à 0 les positions en excès
            for tk in active_long.index:
                if tk not in keep_long:
                    weights[tk] = 0.0
            for tk in active_short.index:
                if tk not in keep_short:
                    weights[tk] = 0.0
            active_long  = weights[weights > 0]
            active_short = weights[weights < 0]

        # Re-normaliser : Σ long = +1, Σ short = -1
        if len(active_long) > 0:
            sum_long = active_long.sum()
            if sum_long != 0:
                weights[active_long.index] = active_long / sum_long
        if len(active_short) > 0:
            sum_short = active_short.abs().sum()
            if sum_short != 0:
                weights[active_short.index] = -active_short.abs() / sum_short

        # Mettre à jour les listes pour le comptage
        long_list  = weights[weights > 0].index.tolist()
        short_list = weights[weights < 0].index.tolist()
